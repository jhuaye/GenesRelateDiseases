...
class BoostedTreeModel(PrepareDatasetDiseaseHie):
    ...
    def train(self, train_p, test_p=None, lc_file=None):
        ...
        classes.columns = ["".join(c if c.isalnum() else "_" for c in str(x)) for x in classes.columns]############
...
"""
    A multilabel light gbm classifier.
"""
class LRModel(PrepareDatasetDiseaseHie):
    ...
    def train(self, train_p, test_p=None, lc_file=None):
        ...
        clf_multilabel = OneVsRestClassifier(LogisticRegression(penalty='l2'))
        print("#############################################")
        clf_multilabel.fit(features, classes)
        self.model = clf_multilabel
...
class ModularModel(PrepareDatasetDiseaseHie, DeepNet):
    ...
    def train(self, train_p, test_p=None, lc_file=None):
        ...
        # builds the DNN
        # print("**************"+str(len(features.columns)))
        # input_layer = Input((len(features.columns),), name='main_input')
        input_layer = Input((len(features.columns),), name='main_input')


        # x = Convolution1D(1, 100, strides=3, padding='valid', use_bias=True, activation='relu')(input_layer)
        # x=Convolution1D(5, 50, strides=2,padding="valid",use_bias=True,activation="relu")(x)
        # x= MaxPooling1D(pool_size=3, padding='valid')(x)
        #
        # x= Flatten()(x)
        x = Dense(64, kernel_initializer='uniform', use_bias=True, activation='relu')(input_layer)
        x = Dropout(0.5)(x)
        x = Dense(32, kernel_initializer='uniform', use_bias=True, activation='relu')(x)
        x = Dropout(0.5)(x)
        x = Dense(32, kernel_initializer='uniform', use_bias=True, activation='relu')(x)
        x = Dropout(0.5)(x)
        x = Dense(16, kernel_initializer='uniform', use_bias=True, activation='relu')(x)
        ...
        output = Dense(len(classes.columns), kernel_initializer='uniform', activation='tanh', name="output")(x)
...
class JoinedModel(PrepareDatasetDiseaseHie, DeepNet):
    ...
    def train(self, train_p, test_p=None, lc_file=None):
        ...
        x = Dense(32, kernel_initializer='uniform', activation='relu', use_bias=True)(input_layer)
        x = Dropout(0.5)(x)
        x = Dense(24, kernel_initializer='uniform', activation='relu', use_bias=True)(x)
        x = Dropout(0.5)(x)
        x = Dense(16, kernel_initializer='uniform', activation='relu', use_bias=True)(x)
        self.projection_layer = Model(input_layer, x)
        x = Dropout(0.5)(x)
        ...
        features = self.scaler.transform(features)
        model.fit(features, classes, epochs=self.epochs, batch_size=512*6,  verbose=0,
                  callbacks=[CSVLogger(lc_file + "_noprior")], validation_data=(test_f, test_c))
