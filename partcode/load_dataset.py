    ...
    def clear_features(features, binary_features):
        ...
        if binary_features:
            ...
            for row in to_remove.index:
                print("WARNING: Removing feature '" + str(row) + "' due to very low < 4 sum.")
                features.drop(row, axis=1, inplace=True)

        else:
            stds = features.std(axis=0)

            to_remove = stds[stds < 1.0e-3]
            for row in to_remove.index:
                print("WARNING: Removing feature '" + row + "' due to very low < 1.0e-5 standard deviation.")
                features.drop(row, axis=1, inplace=True)
        print(features.shape)
